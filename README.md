...
If you don't know your org or workspace IDs yet, leave them blank. Set `KOGNITOS_TOKEN`, `KOGNITOS_REGION`, and `KOGNITOS_ENV`, then run `kognitos-bootstrap` — it discovers the IDs from your token.

### 3. API base URL reference

The base URL pattern is `https://app.<region>-<az>[.<env>].kognitos.com`:
...
The skills pick the right base URL from `KOGNITOS_REGION` and `KOGNITOS_ENV`.
