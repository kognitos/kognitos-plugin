...
curl -sS \
  ...
  "${KOGNITOS_BASE_URL}/api/v1/organizations/${ORG}/workspaces/${WS}/agents/quill/threads"
```
...
curl -sS -X POST \
  ...
  -H "Content-Type: application/json" \
  -d '{}' \
  "${KOGNITOS_BASE_URL}/api/v1/${THREAD_NAME}:resumeStream"
```
...
curl -sS \
  -H "Authorization: Bearer ${KOGNITOS_TOKEN}" \
  "${KOGNITOS_BASE_URL}/api/v1/${THREAD_NAME}/messages?page_size=50"
```
